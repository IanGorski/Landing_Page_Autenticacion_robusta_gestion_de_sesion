...
async function waitForStatus(regex: RegExp, attempts = 20) {
  ...
  console.log(
    '[integration] waitForStatus failed for',
    regex,
    'DOM snapshot:',
...
describe('AuthFlow integration', () => {
  ...
  // Eliminado log de integración
    ...
    if (!extendBtn) {
      console.log(
        '[integration] No se encontró botón Extender sesión. DOM:',
        document.body.innerHTML
      );
    ...
    for (let elapsed = 0; elapsed <= maxSimulated; elapsed += step) {
      ...
      const wrapper = document.querySelector(
        '[data-component="SessionPanel"] .session-countdown-wrapper'
      ) as HTMLElement | null;
      ...
      if (remaining !== undefined)
        console.log('[integration] remainingMs=', remaining, 'elapsed=', elapsed);
      if (remaining !== undefined) {
        if (remaining > lastRemaining) {
          console.log(
            '[integration][warn] remainingMs aumentó inesperadamente',
            lastRemaining,
            '->',
            remaining
          );
    ...
    if (!expiredToast) {
      console.log(
        '[integration] No se encontró toast de expiración tras',
        maxSimulated,
        'ms. DOM final:',
        document.body.innerHTML
      );
